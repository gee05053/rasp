...
try:
    while True:
        ...
        if get_tracking == [1, 0, 0, 1, 1]:
            ...
            car.curve_turn("L", 15, 17, 0.02)
        elif get_tracking == [1, 1, 0, 0, 1]:
            car.curve_turn("R", 25, 27, 0.02)
        elif get_tracking == [1, 1, 1, 0, 1]:
            car.curve_turn("R", 15, 17, 0.02)

        # right turn or stop
        elif get_tracking == [1, 1, 0, 0, 0] or get_tracking == [1, 0, 0, 0, 0] or get_tracking == [0, 0, 0, 0, 0] or get_tracking == [1,1,1,0,0]:
            car.run("F", 35, 0.6)
        ...
        elif get_tracking == [0, 0, 0, 1, 1] or get_tracking == [0, 0, 0, 0, 1] or get_tracking == [0,0,1,1,1]:
            car.run("F", 35, 0.5)
            get_tracking2 = car.trackingSensor.scan()
            # If there is no line
            if get_tracking2 == [1, 1, 1, 1, 1]:
                car.point_turn("L", 90, 0.3)  # 45도 왼쪽 회전
